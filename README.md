...
| `comfyui` | 0.36.0 | `sources/comfyui/` | `all`. [ComfyUI](https://github.com/comfyanonymous/ComfyUI), node-based Stable Diffusion UI. Upstream is a Python source tree with no Linux release; this stages it in `/usr/lib/comfyui` with a `comfyui` launcher that builds a per-user venv (CPU PyTorch, ~2.5 GB) in `~/.local/share/comfyui` on first run. CPU-only on Asahi. Committed (~8 MB) |
